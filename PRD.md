...
| 混合检索 | 向量（BGE-M3）+ BM25 关键词双路召回，RRF 排名融合（k=60）；BM25 为内存索引，中文按字符 bigram + 单字分词 |
...
| 混合检索 | Hybrid Search：向量语义检索与 BM25 关键词检索双路并行召回，再用 RRF 融合排名，互补覆盖语义泛化与字面精确两类查询 |
| BM25 | 经典词频-逆文档频率关键词评分算法；本系统对 Chroma 全量分块构建内存倒排索引（标题×3、标签×2 字段加权） |
| RRF | Reciprocal Rank Fusion，倒数排名融合：score(d)=Σ 1/(k+rank_i(d))，只依赖名次不依赖分数，规避双通道分数量纲不可比 |
...
| 编号 | 功能名称 | 功能描述 | 优先级 |
|------|---------|---------|--------|
| FR-A01 | 流式多轮对话 | 用户发送自然语言问题，系统经混合检索（向量+BM25 双路召回 RRF 融合 → Rerank 交叉编码精排）构建菜谱候选池后交由 LLM 生成回答，使用 SSE 实时流式输出 | P0 |
...
- **混合检索降级**：BM25 关键词通道失败退化为纯向量序、向量通道失败退化为关键词序；混合整体异常或 `RAG_HYBRID_SEARCH=false` 时回退纯向量链路；BM25 索引随向量同步/删除/重建自动失效重建
...
| **参与者** | 普通用户 |
| **前置条件** | 用户已登录 |
| **主流程** | 1. 用户打开 AI 助手对话框<br>2. 输入问题（如"鸡蛋怎么打发更稳定？"）<br>3. 前端调用 `POST /api/ai/chat`，SSE 接收流式回复<br>4. 后端按需加载会话历史到内存（仅内存缓存未命中时），保存用户消息<br>5. 通过 RAG 混合检索相关菜谱/心得作为上下文（向量+BM25 双路召回 RRF 融合→去重→热门补齐→Rerank 精排融合→忌口/预算约束）<br>6. LLM 流式生成回答，逐 chunk 推送<br>7. 流结束后保存 AI 回复到 `ai_messages` 表 |
...
用户查询（AI 对话入口）
   ...
   ▼
┌────────────────────────────────────────────┐
│ 混合召回（双路并行，RAG_HYBRID_SEARCH=true）     │
│ ├─ 向量通道：BGE-M3 嵌入 → Chroma 余弦相似度检索 │ 语义泛化（"下饭菜"≈米饭配菜）
│ └─ 关键词通道：BM25 评分检索（内存索引）          │ 字面精确（菜名/食材整词命中）
│                                              │ 索引文本=标题×3+标签×2+正文（字段加权）
│ 两路各取 top_k → RRF 排名融合：Σ 1/(k+rank_i)   │ 只看名次不看分数，规避量纲不可比
└─────────┬──────────────────────────────────┘  （k=60；任一通道失败退化为单通道）
          │
          ├─两路均失败/关闭混合 ─→ 纯向量检索；仍无结果→降级：拉取元数据+原文，
          │                     关键词 n-gram 评分排序（整词：标题+3.0/标签+1.5/正文+0.8）
          ▼
┌─────────────────────┐
│ 按 source_id 去重     │
└─────────┬───────────┘
...
混合检索实测效果（20 条分层评测集，`eval_hybrid_results.md`）：精确型（菜名）查询
Recall 0.944→1.000、MRR 0.833→0.917；全量 MRR 0.410→0.438、NDCG 0.394→0.411
（α=0.5 生产配置下）。BM25 索引（6608 分块）构建约 0.5s 且全程内存缓存，
随向量同步/删除/重建自动失效重建。
...
| RAG_CHAT_RECALL_K | 40 | 对话RAG每路召回候选分块数（召回率优先） |
| RAG_CHAT_MAX_DISHES | 12 | 去重后喂给模型的菜谱上限 |
| RAG_HYBRID_SEARCH | true | 混合检索开关：向量+BM25 双路召回 RRF 融合；false 回退纯向量召回 |
| RAG_HYBRID_RRF_K | 60 | RRF 平滑常数：score=Σ 1/(k+rank_i)，业界标准 60 |
