...
| 查询改写 | 无实体词查询（与全库菜名无 ≥3 字连续命中）经 LLM 补全菜名/食材实体，与原查询双路召回再经 RRF 融合；改写结果须含库内真实菜名且保留预算约束，否则静默回退原查询 |
...
| 查询改写 | Query Rewriting：查询不含实体词时先经 LLM 改写为含具体菜名/食材的检索表达，与原查询双路召回融合，补足"快手家常菜"这类纯需求描述的召回短板 |
...
| 编号 | 功能名称 | 功能描述 | 优先级 |
|------|---------|---------|--------|
| FR-A01 | 流式多轮对话 | 用户发送自然语言问题，系统经查询改写（无实体词查询经 LLM 补全菜名/食材，与原查询双路召回 RRF 融合）与混合检索（向量+BM25 双路召回 RRF 融合 → Rerank 交叉编码精排）构建菜谱候选池后交由 LLM 生成回答，使用 SSE 实时流式输出 | P0 |
...
| FR-REC03 | 自然语言推荐 | 用户输入自然语言描述，系统通过 RAG 检索菜谱并返回 top-K 候选；用户输入属自写文本，故查询改写默认生效——不含实体词时（如"想给家人做点清淡的"）追加一次 LLM 改写调用（+2.5s 级），改写失败自动回退原查询（系统生成的偏好标签查询则显式关闭改写，不产生额外调用） | P0 |
...
| 查询改写（仅无实体词查询触发一次 LLM） | 实测均值 2.49s | 改写位于召回前置、同步计入 AI 首包延迟；仅"原本检索不到相关菜谱"的查询承担，失败/超时自动回退原查询 |
...
**性能基准实测**（本机 localhost、uvicorn 单 Worker，脚本 `import_data/bench_api.py`，完整结果见论文 6.5 节）：非 AI 内容接口（菜谱列表/搜索/详情）平均响应 11.5~26.8 ms、P99 不超过 60 ms，满足 ≤300ms 目标；登录接口平均 190.6 ms（瓶颈为 bcrypt 慢哈希校验）；AI 对话首包平均 2.60 s、最快 1.53 s，耗时主要来自 LLM 服务端首 Token 排队与解码（据同类接口推算，本地检索与向量化合计约 0.4 s，未单独埋点实测），处于交互可接受区间，进一步改善依赖 LLM 服务侧或本地化部署（该组首包实测早于查询改写上线；改写默认开启后，已含实体词的查询首包不受影响，无实体词查询需追加一次改写调用、首包相应增加约 2.5 s，详见上表"查询改写"行）；首页统计缓存命中平均 1.5 ms（冷启动 7.7 ms）；菜谱列表接口 10 并发下达 100.4 QPS 且错误率 0，50 并发时进入过载区（P95 1304 ms、错误率仍为 0），印证多 Worker 部署可进一步提升并发上限。
...
- **查询改写降级**：改写 LLM 调用失败/超时（`RAG_QUERY_REWRITE_TIMEOUT`，默认 10s）、输出为空或与原查询相同、校验不过（含幻觉菜名或丢失预算约束）时静默回退原查询，不改写也不抛异常；未含实体词的查询才触发改写，其余查询零 LLM 开销；可通过 `RAG_QUERY_REWRITE=false` 一键关闭
...
| **参与者** | 普通用户 |
| **前置条件** | 用户已登录 |
| **主流程** | 1. 用户打开 AI 助手对话框<br>2. 输入问题（如"鸡蛋怎么打发更稳定？"）<br>3. 前端调用 `POST /api/ai/chat`，SSE 接收流式回复<br>4. 后端按需加载会话历史到内存（仅内存缓存未命中时），保存用户消息<br>5. 通过 RAG 检索相关菜谱/心得作为上下文（查询改写：无实体词查询经 LLM 补全菜名/食材后与原查询双路召回 RRF 融合 → 混合检索：向量+BM25 双路召回 RRF 融合 → 去重 → 热门补齐 → Rerank 精排融合 → 忌口/预算约束）<br>6. LLM 流式生成回答，逐 chunk 推送<br>7. 流结束后保存 AI 回复到 `ai_messages` 表 |
...
### 5.4 RAG 查询改写、混合检索与两阶段精排流程（对话场景）
...
用户查询（AI 对话入口）
   ...
   ▼
┌────────────────────────────────────────────┐
│ 查询改写（RAG_QUERY_REWRITE=true，可选前置）    │ 触发判定：查询与全库菜名无 ≥3 字连续命中
│  无实体词 → LLM 补全具体菜名/食材实体           │ 已含实体词（"清蒸鲈鱼的做法"）→ 跳过，零 LLM 开销
│  校验：须含库内真实菜名 + 保留预算金额           │ 幻觉菜名/约束丢失/超时 → 静默回退原查询
│  改写查询与原查询双路召回 → RRF 融合            │ 原查询召回能力始终保留，改写只做增量补充
└─────────┬──────────────────────────────────┘
          │
          ▼
┌────────────────────────────────────────────┐
...
混合检索实测效果（20 条分层评测集，`eval_rewrite_results.md`，2026-09-18 运行）：精确型（菜名）查询
Recall 0.944→1.000、MRR 0.833→0.917；全量 MRR 0.411→0.438、NDCG 0.395→0.411
（α=0.5 生产配置下）。BM25 索引（6608 分块）构建约 0.5s 且全程内存缓存，
随向量同步/删除/重建自动失效重建。

查询改写实测效果（同一评测集，`eval_rewrite_results.md`）：20 条查询中 12 条触发改写、
全部改写成功，改写 LLM 平均耗时 2.49s（仅触发查询承担）；全量 Recall@12 0.483→0.575
（+9.2pp，相对 +19.0%）、NDCG@12 0.411→0.471，场景意图型查询由 0→0.133（5 条中 2 条
相关菜进入前 12）、类别型 0.444→0.542、约束型 0.333→0.528，精确型持平（触发判定全部跳过）。
...
| RAG_QUERY_REWRITE | true | 查询改写开关：无实体词查询经 LLM 补全菜名/食材，与原查询双路召回 RRF 融合；false 关闭（零 LLM 调用） |
| RAG_QUERY_REWRITE_TIMEOUT | 10 | 改写 LLM 调用超时（秒），超时/失败/校验不过静默回退原查询 |
