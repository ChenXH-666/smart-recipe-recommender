    ...
    <el-card shadow="never" class="section-card">
      <el-table :data="items" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column label="成本" width="120" align="center">
          <template #default="{ row }">
            <el-tag v-if="row.estimated_cost" type="warning" effect="plain" size="small">
              ¥{{ row.estimated_cost }}
            </el-tag>
        ...
        <el-table-column label="操作" width="290" align="center">
          <template #default="{ row }">
            <el-button type="primary" link size="small" @click="$router.push(`/recipes/${row.id}`)">
...
async function load() {
  ...
  try {
    ...
    const res = await admin.pendingRecipes(params)
...
async function confirmAudit() {
  ...
  try {
    await admin.auditRecipe(auditId.value, {
    ...
    // 错误已由全局拦截器提示，此处捕获避免未处理的 Promise 拒绝
  } finally {
...
async function del(id) {
  try {
    await ElMessageBox.confirm('确定删除该菜谱？此操作不可撤销。', '提示', { type: 'warning' })
  } catch (e) {
    return  // 用户取消
  }
  try {
    await admin.removeRecipe(id)
