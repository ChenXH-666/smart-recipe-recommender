...
【查询改写参数说明（Query Rewriting，召回前置）】
  RAG_QUERY_REWRITE = True → 查询"无实体词"时先经 LLM 改写为含具体菜名/食材的检索表达，
    再与原查询双路召回、RRF 融合。触发条件：查询与全库菜名无 ≥3 字连续片段命中
    （"快手家常菜""减脂晚餐"这类纯需求描述），已含实体词的查询跳过、不调用 LLM；
    改写结果须含库内真实菜名且保留原数值约束，否则静默回退原查询
  RAG_QUERY_REWRITE_TIMEOUT = 10 → 改写 LLM 调用超时秒数，超时即回退原查询，不阻塞主链路
"""
...
class Settings(BaseSettings):
    ...
    # 查询改写（Query Rewriting）：仅对"无实体词"的口语化查询触发一次 LLM 改写，
    # 改写结果与原查询双路召回、RRF 融合（原查询召回能力始终保留）。
    # 触发判定：查询与全库菜名无 ≥3 字连续片段命中（如"快手家常菜""减脂晚餐"）；
    # 已含实体词（"清蒸鲈鱼的做法"）直接跳过，不产生 LLM 调用开销。
    # 改写结果须含库内真实菜名、保留原数值约束，否则静默回退原查询。
    RAG_QUERY_REWRITE: bool = True
    RAG_QUERY_REWRITE_TIMEOUT: int = 10      # 改写 LLM 调用超时（秒），超时回退原查询
